...
**The icons were drifting with zoom, and the cause was measurable.** Every glyph
had its *bounding box* centred but its *ink* off centre by some residual, and a
residual expressed in viewBox units scales with the render size. A glyph sitting
0.35 units off is a third of a pixel out at 100% and two thirds at 200%: the
mark appears to slide inside its circle as the page is zoomed, and because each
icon carried a different residual, they slide by different amounts in different
directions.

Measured before the fix, in viewBox units:

    YouTube   -0.04, -0.02      TikTok    -0.22, +0.29
    Instagram +0.11, -0.16      LinkedIn  -0.14, +0.31
    X         -0.11, +0.01      Telegram  +0.35, -0.14
    ic-sun    -0.02, -0.03      ic-moon   -0.78, +0.72

The moon was the worst by a factor of two, which also made the theme toggle the
most visible case: sun and moon sit in the same button, the sun was dead centre,
and the two states therefore did not agree with each other. That was the
previous pass's fault. It split the difference between the moon's box centre and
its mass centre, which is the right call for a glyph judged on its own and the
wrong one for a glyph that has to agree with the icon it swaps places with.

The fix is one rule applied to all eight: the ink centroid goes on (12, 12),
solved iteratively by rasterising each icon and nudging its transform until the
measurement converges. Iteratively because the centroid of a stroked shape has
no closed form in terms of its transform, since the stroke is laid on after the
geometry. Telegram and TikTok are asymmetric enough that centring the ink pushed
a tip close to the frame, so both come off their optical cap by a few percent to
buy the clearance back.

`shape-rendering="geometricPrecision"` is now set on all eight, which stops the
renderer applying the snapping heuristics that `auto` permits and that differ
between zoom levels. And the icon sizes moved to 1.25rem in the 2.25rem footer
button and 1.125rem in the 2.5rem toggle, so the leftover space divides to a
whole 8px and 11px at 100% rather than 8.4px and 11.6px.
